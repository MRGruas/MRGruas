...
(function () {
  function C () {
    if (!D && document.body) {
      ...
      k = document.compatMode.indexOf('CSS') >= 0 ? b : a
      m = a
  ...
  function N (a, b, d) {
    ea(b, d)
    if (f.accelerationMax != 1) {
      var c = Date.now() - E
      c < f.accelerationDelta &&
        (
          (c = (1 + 50 / c) / 2),
          c > 1 && ((c = Math.min(c, f.accelerationMax)), (b *= c), (d *= c))
        )
      E = Date.now()
    }
    t.push({
    ...
    if (!F) {
      ...
      var h = function (c) {
        ...
        for (var g = 0, k = 0, l = 0; l < t.length; l++) {
          var n = t[l]
          var p = c - n.start
          var m = p >= f.animationTime
          var q = m ? 1 : p / f.animationTime
          f.pulseAlgorithm &&
            (
              (p = q),
              p >= 1
                ...
                : p <= 0
                  ? (q = 0)
                  : (
                    f.pulseNormalize == 1 && (f.pulseNormalize /= P(1)),
                    (q = P(p))
                  )
            )
          p = (n.x * q - n.lastX) >> 0
  ...
  function R (a) {
    ...
    var c = -a.wheelDeltaY || a.deltaY || 0
    ga &&
      (
        a.wheelDeltaX &&
          x(a.wheelDeltaX, 120) &&
    ...
    Math.abs(d) > 1.2 && (d *= f.stepSize / 120)
    Math.abs(c) > 1.2 && (c *= f.stepSize / 120)
    N(b, d, c)
    a.preventDefault()
    T()
  }
  function M (a) {
    ...
    var e = /^(button|submit|radio|checkbox|file|color|image)$/i
    if (
      !(c =
        a.defaultPrevented ||
        c.test(b.nodeName) ||
    ...
    if (
      ...
      (r(b, 'input') && b.type == 'radio' && ia[a.keyCode])
    ) {
      return !0
    }
    c = b = 0
  ...
  function X (a) {
    return getComputedStyle(a, '').getPropertyValue('overflow-y') !== 'hidden'
  }
  function Y (a) {
  ...
  function ha (a) {
    if (a) {
      return l.length || (l = [a, a, a]), (a = Math.abs(a)), l.push(
        a
      ), l.shift(), clearTimeout(Z), (Z = setTimeout(function () {
        try {
          localStorage.SS_deltaBuffer = l.join(',')
        } catch (a) {}
      }, 1e3)), (a = a > 120 && K(a)), !K(120) && !K(100) && !a
    }
  }
  function x (a, b) {
  ...
  var l = []
  var Z
  var ga = /^Mac/.test(navigator.platform)
  var h = {
    left: 37,
    up: 38,
    right: 39,
  ...
  var u = /Edge/.test(g)
  var G = /chrome/i.test(g) && !u
  var u = /safari/i.test(g) && !u
  var ja = /mobile/i.test(g)
  var ka = /Windows NT 6.1/i.test(g) && /rv:11/i.test(g)
  var ca = u && (/Version\/8/i.test(g) || /Version\/9/i.test(g))
  var g = (G || u || ka) && !ja
  var ba = !1
  try {

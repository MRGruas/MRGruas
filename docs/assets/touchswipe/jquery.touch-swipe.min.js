...
!(function (factory) {
  ...
  function init (options) {
    return !options ||
      ...
      (options.allowPageScroll = NONE), void 0 !== options.click && void 0 === options.tap && (options.tap = options.click), options || (options = {}), (options = $.extend({}, $.fn.swipe.defaults, options)), this.each(
      function () {
        var $this = $(this)
  ...
  function TouchSwipe (element, options) {
    function touchStart (jqEvent) {
      if (
        ...
        if (
          ...
          options.fallbackToMouseEvents != 0
        ) {
          var ret
          var touches = event.touches
          var evt = touches ? touches[0] : event
          return (phase = PHASE_START), touches
    ...
    function removeListeners () {
      $element.unbind(
        START_EV,
        touchStart
      ), $element.unbind(CANCEL_EV, touchCancel), $element.unbind(MOVE_EV, touchMove), $element.unbind(END_EV, touchEnd), LEAVE_EV && $element.unbind(LEAVE_EV, touchLeave), setTouchInProgress(!1)
    }
      ...
      return !validTime || didCancel
        ...
        : !validDistance ||
          ...
          ? !validDistance &&
            ...
            options.triggerOnTouchLeave &&
            (nextPhase = PHASE_CANCEL)
          : (nextPhase = PHASE_END), nextPhase
    }
    function triggerHandler (event, phase) {
      var ret
      var touches = event.touches
      return (didSwipe() || hasSwipes()) &&
        (ret = triggerHandlerForGesture(
          event,
          phase,
          SWIPE
        )), (didPinch() || hasPinches()) && ret !== !1 && (ret = triggerHandlerForGesture(event, phase, PINCH)), didDoubleTap() && ret !== !1 ? (ret = triggerHandlerForGesture(event, phase, DOUBLE_TAP)) : didLongTap() && ret !== !1 ? (ret = triggerHandlerForGesture(event, phase, LONG_TAP)) : didTap() && ret !== !1 && (ret = triggerHandlerForGesture(event, phase, TAP)), phase === PHASE_CANCEL && touchCancel(event), phase === PHASE_END && (touches ? touches.length || touchCancel(event) : touchCancel(event)), ret
    }
    ...
    function validateSwipeDistance () {
      ...
      return options.threshold !== null &&
        (valid = distance >= options.threshold), valid
    }
    function didSwipeBackToCancel () {
      var cancelled = !1
      return options.cancelThreshold !== null &&
        direction !== null &&
        (cancelled =
    ...
    function validatePinchDistance () {
      return options.pinchThreshold !== null
    ...
    function createFingerData (id, evt) {
      ...
      return (f.start.x = f.last.x = f.end.x =
        evt.pageX ||
        evt.clientX), (f.start.y = f.last.y = f.end.y = evt.pageY || evt.clientY), (fingerData[id] = f), f
    }
    function updateFingerData (evt) {
      ...
      return f === null &&
        (f = createFingerData(
          id,
          evt
        )), (f.last.x = f.end.x), (f.last.y = f.end.y), (f.end.x = evt.pageX || evt.clientX), (f.end.y = evt.pageY || evt.clientY), f
    }
    ...
    function createMaximumsData () {
      ...
      return (maxData[LEFT] = createMaximumVO(
        LEFT
      )), (maxData[RIGHT] = createMaximumVO(RIGHT)), (maxData[UP] = createMaximumVO(UP)), (maxData[DOWN] = createMaximumVO(DOWN)), maxData
    }
    ...
    function calculateAngle (startPoint, endPoint) {
      ...
      var angle = Math.round(180 * r / Math.PI)
    ...
    (this.enable = function () {
      return this.disable(), $element.bind(START_EV, touchStart), $element.bind(
        CANCEL_EV,
        touchCancel
      ), $element

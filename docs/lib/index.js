...
const replaceConfig = (src, { path }) => {
  ...
  const sort = ([keya, { deprecated: depa }], [keyb, { deprecated: depb }]) => {
    return depa && !depb ? 1 : !depa && depb ? -1 : localeCompare(keya, keyb);
  };

  const allConfig = Object.entries(definitions)
    .sort(sort)
...
const replaceShorthands = (src, { path }) => {
  ...
  const sh = Object.entries(shorthands)
    .sort(
      ([shorta, expansiona], [shortb, expansionb]) =>
        // sort by what they're short FOR
        localeCompare(expansiona.join(" "), expansionb.join(" ")) ||
        localeCompare(shorta, shortb)
    )
    .map(([short, expansion]) => {
...
const transformMan = (src, { data, unified, remarkParse, remarkMan }) =>
  unified()
    ...
    .use(remarkMan)
    .processSync(
      `# ${data.title}(${data.section}) - ${data.description}\n\n${src}`
    )
    .toString();

const manPath = (name, { data }) =>
  join(`man${data.section}`, `${name}.${data.section}`);

const transformMd = (src, { frontmatter }) =>
  ["---", frontmatter, "---", "", src].join("\n");
